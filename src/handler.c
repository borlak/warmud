...
int get_curr_str( CHAR_DATA *ch )
{
    ...
    return URANGE( 3, class_table[ch->class].stat_str + ch->pcdata->mod_str, 25 );
}
...
int get_curr_int( CHAR_DATA *ch )
{
    ...
    return URANGE( 3, class_table[ch->class].stat_int + ch->pcdata->mod_int, 25 );
}
...
int get_curr_wis( CHAR_DATA *ch )
{
    ...
    return URANGE( 3, class_table[ch->class].stat_wis + ch->pcdata->mod_wis, 25 );
}
...
int get_curr_dex( CHAR_DATA *ch )
{
    ...
    return URANGE( 3, class_table[ch->class].stat_dex + ch->pcdata->mod_dex, 25 );
}
...
int get_curr_con( CHAR_DATA *ch )
{
    ...
    return URANGE( 3, class_table[ch->class].stat_con + ch->pcdata->mod_con, 25 );
}
...
void extract_obj( OBJ_DATA *obj )
{
    ...
    /* remove from spec list */
    if( obj->spec_obj != NULL )
    {
	    SPEC_OBJ_DATA *spec;
	    SPEC_OBJ_DATA *spec_next;

	    for( spec = obj->spec_obj; spec; spec = spec_next )
	    {
		    spec_next		= spec->next;
	    ...
	    if( obj_spec_list == obj )
	    {
		    obj_spec_list = obj->next_spec;
	    }
	    else
	    {
		    OBJ_DATA *prev;

		    for( prev = obj_spec_list; prev; prev = prev->next_spec )
		    {
			    if( prev->next_spec == obj )
			    {
				    prev->next_spec = obj->next_spec;
				    break;
			    }
		    }
	    }
...
void extract_char( CHAR_DATA *ch, bool fPull )
{
    ...
    if( ch->spec_obj != NULL )
    {
	    ...
	    for( spec = ch->spec_obj; spec; spec = spec_next )
	    {
		    spec_next		= spec->next;
		    spec->next		= spec_obj_free;
		    spec_obj_free	= spec;
	    ...
	    else
	    {
		    ...
		    for( prev = mob_spec_list; prev; prev = prev->next_spec )
		    {
			    if( prev->next_spec == ch )
			    {
				    prev->next_spec = ch->next_spec;
				    break;
			    }
		    }
	    }
...
CHAR_DATA *get_char_room( CHAR_DATA *ch, char *argument )
{
	...
	|| ( IS_NPC(rch) && !is_name(arg, rch->name) ) )
		continue;
	if ( !IS_NPC(rch) && str_prefix(arg,rch->name) )
		continue;
...
bool can_see( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    && !IS_SET(class_table[ch->class].flags, CLS_INFRAVISION)
    && !IS_SET(class_table[ch->class].flags, CLS_ULTRAVISION) )
	return FALSE;

    if ( war_type == WAR_HOT_SKULL )
	    return TRUE;
...
char *affect_bit_name( int vector )
{
    ...
    if ( vector & AFF_BLACKMANTLE   ) strcat( buf, " blackmantle"   );
...
char *extra_bit_name( int extra_flags )
{
    ...
    if ( extra_flags & ITEM_MISSILE_WEAPON)strcat(buf,  " missile-weapon" );
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
