...
void do_give( CHAR_DATA *ch, char *argument )
{
    ...
    update_specs( ch->in_room, (void*)victim, (void*)obj, NULL, UPDATE_ALL, UPDATE_GIVE );
...
void do_drink( CHAR_DATA *ch, char *argument )
{
	...
	/* heal, hurt */
	heal_char	( ch, liq_table[liquid].heal_hp );
	ch->mana	+= UMIN( ch->max_mana, ch->mana+liq_table[liquid].heal_mana );
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace )
{
    ...
    if ( IS_SET(class_table[ch->class].flags, CLS_NO_EQUIP) )
    {
	    send_to_char("You refuse to use such implements.\n\r", ch );
	    return;
    }

    if ( IS_SET(obj->extra_flags, ITEM_MAGIC)
    &&   IS_SET(class_table[ch->class].flags, CLS_NO_MAGIC) )
    {
	    send_to_char("You can't use magical items.\n\r", ch );
	    return;
    }
    ...
    if( CAN_WEAR( obj, ITEM_WEAR_QUIVER) )
    {
	    if( !remove_obj( ch, WEAR_QUIVER, fReplace ) )
		    return;
	    act("$n slings $p on $s back.",	ch, obj, NULL, TO_ROOM );
	    act("You sling $p on your back.",	ch, obj, NULL, TO_CHAR );
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    {
	act( "$n offers $mself to Borlak, who graciously declines.",
	    ch, NULL, NULL, TO_ROOM );
	send_to_char(
	    "Borlak appreciates your offer and may accept it later.", ch );
	return;
    }
    ...
    if ( !CAN_WEAR(obj, ITEM_TAKE) && (!IS_SET(war_flags,WFLAG_BLOODS) && obj->item_type == ITEM_CORPSE_PC) )
    {
	act( "$p is not an acceptable sacrifice.", ch, obj, 0, TO_CHAR );
	return;
    }

    act( "$n sacrifices $p to Borlak.", ch, obj, NULL, TO_ROOM );
    act( "You sacrifice $p to Borlak.", ch, obj, NULL, TO_CHAR );
    extract_obj( obj );

    if( IS_SET(war_flags,WFLAG_BLOODS) && obj->item_type == ITEM_CORPSE_PC )
    {
	obj = create_object( get_obj_index(OBJ_VNUM_BLOOD) );
	send_to_char("Borlak gives you a &+rquart of fresh blood&n for your sacrifice.\n\r", ch );
	obj_to_char(obj,ch);
	return;
    }
    return;
...
void do_steal( CHAR_DATA *ch, char *argument )
{
    ...
    WAIT_STATE( ch, GET_BEAT_SKILL(ch,gsn_steal)  );

    percent  = number_percent( ) + ( IS_AWAKE(victim) ? 10 : -50 );

    if ( victim->position == POS_FIGHTING
    ||   number_percent() > GET_PRAC_SKILL(ch,gsn_steal) )
    {
	/*
	 * Failure.
	 */
