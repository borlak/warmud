...
/* Hunt.c added by Borlak for pkmud II */
...
void do_track( CHAR_DATA *ch, char *argument )
{
	...
	if( GET_PRAC_SKILL(ch, gsn_track) <= 0 )
	{
		send_to_char("You don't even know how to track!\n\r", ch );
		return;
	}
	...
	{
		send_to_char("You are too exhausted from your last track...wait a minute.\n\r", ch );
		return;
	}

	ch->pcdata->timer = 1;
	act( "$n carefully sniffs the air.", ch, NULL, NULL, TO_ROOM );
	WAIT_STATE(ch, GET_BEAT_SKILL(ch,gsn_track) );
	...
	{
		send_to_char( "Hmm... Something seems to be wrong. Tell Borlak.\n\r", ch );
		return;
	}
	 ...
	 */
	if( number_percent() > GET_PRAC_SKILL(ch,gsn_track) )
	{
		do
		{
	 ...
	 */
	WAIT_STATE( ch, GET_BEAT_SKILL(ch,gsn_track));
	sprintf( buf, "&+W$N&N is &+g%s&N from here.", dir_name[direction] );
