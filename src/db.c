...
int			greet_text_num	= 0;
int			greet_ansi_num	= 0;
char			log_buf		[2*MAX_INPUT_LENGTH];
KILL_DATA		kill_table	[MAX_LEVEL];
NOTE_DATA *		note_list;
OBJ_DATA *		object_list;

CHAR_DATA *		mob_spec_list;
OBJ_DATA *		obj_spec_list;
ROOM_INDEX_DATA *	room_spec_list;
...
void	check_mountable		args( ( MOB_INDEX_DATA *mob ) );
void	check_special_weapon	args( ( OBJ_INDEX_DATA *obj ) );
...
void load_objects( FILE *fp )
{
	...
	/* is special type of weapon? bow, quiver, etc. */
	check_special_weapon( pObjIndex );
...
void load_specials( FILE *fp )
{
	...
	case 'M':
		...
		spec->spec_fun		= spec_lookup		( fread_word   ( fp ) );
		if( spec->spec_fun == 0 )
		{
			bug( "Load_specials: 'M': vnum %d.", pMobIndex->vnum );
			exit( 1 );
		}
		spec->value[0]		= fread_number(fp);
		spec->value[1]		= fread_number(fp);
		spec->value[2]		= fread_number(fp);
		spec->value[3]		= fread_number(fp);
		spec->next		= pMobIndex->spec_obj;
		pMobIndex->spec_obj	= spec;
		break;
	case 'O':
		spec = new_spec_obj();
		...
		spec->value[2]		= fread_number(fp);
		spec->value[3]		= fread_number(fp);
		spec->next		= pObjIndex->spec_obj;
		pObjIndex->spec_obj	= spec;
		break;
	case 'R':
		spec = new_spec_obj();
		...
		if( spec->spec_fun == 0 )
		{
			bug("Load_specials: 'R': vnumd %d.", pRoomIndex->vnum );
			exit( 1 );
		}
		...
		spec->value[2]		= fread_number(fp);
		spec->value[3]		= fread_number(fp);
		spec->next		= pRoomIndex->spec_obj;
		pRoomIndex->spec_obj	= spec;
		add_spec( spec_name_lookup(spec->spec_fun), pRoomIndex, NULL, NULL );
		break;
	}
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
    ...
    if( pMobIndex->spec_obj )
    {
	    for( spec = pMobIndex->spec_obj; spec; spec = spec->next )
		    add_spec( spec_name_lookup(spec->spec_fun), NULL, mob, NULL );
    }

    mob->level		= pMobIndex->level/6;
...
OBJ_DATA *create_object( OBJ_INDEX_DATA *pObjIndex )
{
    ...
    *obj		= obj_zero;
    ...
    if( pObjIndex->spec_obj )
    {
	    for( spec = pObjIndex->spec_obj; spec; spec = spec->next )
		    add_spec( spec_name_lookup(spec->spec_fun), NULL, NULL, obj );
    }
...
/**************************Special Checking Procedures**************************/

void check_mountable( MOB_INDEX_DATA *mob )
{
	static const char *mount_names = "horse bull warg dragon";
	char *p = mob->player_name;
	char arg[200];
	...
	do
	{
		...
		if( strstr(mount_names, arg) != NULL )
			SET_BIT(mob->act, ACT_MOUNTABLE);

	} while( arg && arg[0] != '\0' );
}


void check_special_weapon( OBJ_INDEX_DATA *obj )
{
	static const char *bow_names		= "bow longbow shortbow short-bow long-bow";
	static const char *quiver_names		= "quiver arrow arrows";
	static const char *lance_names		= "lance polearm pole ranseur pike spear halberd";
	static const char *stabber_names	= "dagger short shortsword knife dirk";
	char *p = obj->name;
	char arg[200];
	...
	do
	{
		...
		if( strstr(bow_names,arg) != NULL )
		{
			SET_BIT(obj->extra_flags, ITEM_MISSILE_WEAPON);
			return;
		}
	
		if( strstr(quiver_names,arg) != NULL )
		{
			obj->wear_flags = ITEM_WEAR_QUIVER|ITEM_TAKE;
			obj->item_type  = ITEM_QUIVER;
			return;
		}
	
		if( strstr(lance_names,arg) != NULL )
		{
			SET_BIT(obj->extra_flags, ITEM_CHARGE_WEAPON);
			return;
		}

		if( strstr(stabber_names,arg) != NULL )
		{
			SET_BIT(obj->extra_flags, ITEM_STABBER);
			return;
		}
