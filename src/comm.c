...
void	game_loop_unix		args( ( unsigned long control ) );
...
void game_loop_unix( unsigned long control )
{
    ...
    sleep_socket = socket(AF_INET, SOCK_STREAM, 0);
	 ...
	 */
	{
		...
		FD_ZERO(&sleep_set);
		FD_SET( (unsigned int)sleep_socket, &sleep_set );
		if ( select( 0, NULL, NULL, &sleep_set, &stall_time ) < 0 )
		{
...
bool process_output( DESCRIPTOR_DATA *d, bool fPrompt )
{
	...
	char *buf;
	char mainbuf[MAX_STRING_LENGTH+1];
	int j, k=0, length;
	bool bg, bold, blink, flag = FALSE;
	char prev;
	...
	*/
	if ( fPrompt && !merc_down && d->connected == CON_PLAYING )
	{
		...
		if ( IS_SET(ch->act, PLR_PROMPT) )
		{
			...
			ch = d->character;
			sprintf( buf, "&+g<%dhp %dm>&n ", ch->hit, ch->mana );
	...
	*/
	...
	if( d->character )
		flag = IS_SET(d->character->act, PLR_ANSI) ? TRUE : FALSE;

	buf = d->outbuf;
	length = d->outtop;
	mainbuf[0] = '\0';

	for( j = 0; *buf != '\0' && (j < sizeof(mainbuf)); buf++ )
	{
		if( *buf == '&' )
		{
			buf++;
	...
	* OS-dependent output.
	*/
	if ( !write_to_descriptor( d->descriptor, mainbuf, 0 ) )
	{
		d->outtop = 0;
		return FALSE;
	}
...
/* Lets make this Ansi compatible, eh? *Borlak*/
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
    ...
    extern int greet_text_num;
    extern int greet_ansi_num;
    ...
    case CON_INTRO:
	    switch( argument[0] )
	    {
	    case 's': case 'S': /* skip intro */
		    break;
	    case 'a': case 'A': /* ansi */
		    write_to_descriptor( d->descriptor, help_ansi_greeting[number_range(0,(greet_ansi_num-1))], 0);
		    break;
	    default:		/* ascii */
		    write_to_descriptor( d->descriptor, help_text_greeting[number_range(0,(greet_text_num-1))], 0);
		    break;
	    }
	...
	if ( ch->level == 0 )
	{
	    ...
	    ch->hit	= ch->max_hit;
	    ch->mana	= ch->max_mana;
	    set_title( ch, "the Title-Less Player Killer" );
	    do_changeclass(ch,"mage");
